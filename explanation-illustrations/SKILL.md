...
- **If no API key is set**, do not stop the task. Fall back to **prompt-only mode**: output the final, ready-to-paste image prompt(s) so the user can generate them in their own tool, and clearly say no key was found.
